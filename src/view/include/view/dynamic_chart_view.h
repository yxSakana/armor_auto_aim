...
public:
    ...
    bool getXFixed() const { return m_x_fixed; }
    void setXFixed(bool v) { m_x_fixed = v; }

    bool getYFixed() const { return m_y_fixed; }
    void setYFixed(bool v) { m_y_fixed = v; }
...
;
    ...
    void updateView() {
        ...
        if (!m_x_fixed) {
            ...
            if (!axes.empty()) axes.first()->setRange(m_min_x, m_max_x);
        }
        // y axes range
        ...
        if (!axes.empty()) axes.first()->setRange(m_min_y, m_max_y);
...
private:
    ...
    double m_min_y = 0.f;
    double m_max_y = 0.f;
    bool m_x_fixed = false;
    bool m_y_fixed = false;
