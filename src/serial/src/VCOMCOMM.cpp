...
void VCOMCOMM::portReadyRead() {
    ...
    if (pdata[0] == 0x5a) {
        ...
        uint16_t c = armor_auto_aiming::Verify_CRC16_Check_Sum(array);
        if (len == 0 || c == crc) {
...
void VCOMCOMM::Transmit(uint8_t fun_code, uint16_t id, const QByteArray& data) {
    ...
    *((uint16_t*) (buff + 6 + len)) = (len == 0) ? 0 : armor_auto_aiming::Verify_CRC16_Check_Sum(data);
